...
import { getOutreachTargets, getOutreachQueue } from "../lib/data";
...
export default function OutreachPage({ searchParams }: { searchParams: { slug?: string } }) {
  ...
  if (!me.isAdmin) return (<><PageHead title="AI Outreach" sub="Draft messages to your customers" /><RepNudge what="customers to message" /></>);
  const customers = getOutreachTargets()
    .map((c) => ({
